<?php
    ...
    // Hallamos el criterio de busqueda
    if(is_category()){
        $words = single_cat_title('', false).' category'; // Devuelve la categoria buscada
    }elseif(is_tag()){
        $words = single_tag_title('', false).' tag';
    }elseif(is_author()){
        the_post(); // Esto hace que se desplace el puntero del resultset al segundo resultado
        $words = get_the_author();
        // Debemos devolver el puntero del resultset al primer resultado
        rewind_posts();
    }elseif(is_day()){
        $words = get_the_date(); // Devuelve la fecha a la que has pinchado
    }elseif(is_month()){
